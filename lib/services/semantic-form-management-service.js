...
export class SemanticFormManagementService {
  ...
  constructor(config_files, meta) {
    ...
    this.meta = meta;
    this.source = new SourceDataGenerationService(this.config_files);
  ...
  async getSemanticFormBundle(uuid) {
    ...
    try {
      ...
      let source_definition = this.config_files.config.content['resource'];
      // NOTE: if the semantic-form contains sources, we prefer those.
      if (form.sources.length) {
        source_definition = form.getSource('config.json').content['resource'];
        bundle.specification = form.getSource('form.ttl');
        bundle.meta = form.getSource(META_FILE_MATCHER);
      }

      bundle.source = {content: await this.source.generate(form.uri, source_definition)};
      return {form, bundle};
