...
import { META_FILE_MATCHER } from './meta-data-service';
import { SourceDataGenerationService } from './source-data-generation-service';

/**
 * Service providing all business logic for semantic-forms
 */
export class SemanticFormManagementService {

  constructor(config_files, meta) {
    this.config_files = config_files;
    this.meta = meta;
    this.sourceDataGenerationService = new SourceDataGenerationService(this.config_files);
  ...
  async getSemanticFormBundle(uuid) {
    ...
    try {
      ...
      bundle.specification = this.config_files.specification;
      bundle.meta = this.meta.latest;

      // NOTE: if the semantic-form contains sources, we prefer those.
      if (form.sources.length) {
        bundle.specification = form.getSource('form.ttl');
       ...
       *
       * TODO:   mutation/management service shouldn't be responsible for this,
       *         move this to a 'source-data-generation-service'
       *
       * TODO:  in the future this should be replaced with a function that will construct the source-data
       *        based on the `sh:path`'s defined in the linked form specification.
       */
      bundle.source = {content: await this.sourceDataGenerationService.generate(form.uri, form.getSource('config.json') && form.getSource('config.json').content['resource'])};
      return {form, bundle};
