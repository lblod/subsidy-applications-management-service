...
export class SemanticFormSources {
  ...
  async init() {
    ...
    for (const {name, path} of matches) {
      ...
      for (const [created, sources] of Object.entries(versioned.filesByCreated)) {
        const configuration = new Configuration({name, path, sources});
  ...
  getConfiguration(uri) {
    ...
    if (!configuration) {
      throw`Couldn't find configuration for <${uri}>`;
    }
  ...
  async getMeta(uri, configuration = this.getConfiguration(uri)) {
    ...
    if (!meta) {
      // TODO can't be configuration.name, collision
  ...
  async getLatest(uri) {
    ...
    const meta = this.getMeta(uri, configuration);
