...
export class SemanticFormSources {
  ...
  /**
   * Returns the latest meta {@link SemanticFile} for the given URI.
   *
   * If it can't retrieve a {@link GlobalMeta} process for the given URI, it will spawn one.
   *
   * @param uri physical URI of the {@link SemanticFormSpecification.turtle}
   *
  ...
  async getMeta(uri, configuration = this.getConfiguration(uri)) {
    ...
    if (!meta) {
       ...
       * TODO:
       *
       * The URI and configuration should ALWAYS be linked, aka, the configuration should be for the given URI.
       * If this would not be the case, it could cause unexpected behavior.
       *
       * To ensure integrity it should either check the configuration or always retrieve it.
       */
    ...
    await meta.trigger(); // trigger to notify it has been used (and extent it's lifetime).
    return meta.latest;
  }
