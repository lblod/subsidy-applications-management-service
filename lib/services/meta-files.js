import {
  ...
  META_DATA_EXTRACTION_BACKOFF_MAX_WAIT, TAILORED_META_DATA_ROOT,
} from '../../env';
...
export class MetaFiles {
  ...
  constructor(configuration) {
    ...
    mkDirIfNotExistsSync(META_DATA_ROOT);
  ...
  async sync() {
    ...
    if (attempt) {
      const filename = `${META_DATA_ROOT}${VersionedFile.timestamp()}${META_FILE_MATCHER}`;
      if (DEBUG_LOGS)
  ...
  async extractMetaDataBackOff(previous = '', attempt = 0) {
    ...
    if (timeout < META_DATA_EXTRACTION_BACKOFF_MAX_WAIT) {
      ...
      throw `Creation off meta-data failed. Reason: meta-data is unstable, are there migrations running?`;
    }
  }
}