...
export class MetaFiles {
   ...
   */
  get latest() {
    return this.directory.getLatestFor(META_FILE_MATCHER);
  }
  ...
  async sync() {
    ...
    // NOTE: we set the previous meta-data (contents off the latest meta-file)
    let previous = '';
    ...
    if (attempt) {
      const filename = `${META_DATA_ROOT}${VersionFile.timestamp()}--${uuidv4()}-${META_FILE_MATCHER}`;
      if (DEBUG_LOGS)
    ...
    console.log(`Synchronization off the meta-files succeeded at ${end}. That took me only ${diff.humanize()}!`);
    return this.latest;
  }
  ...
  async extractMetaDataBackOff(previous = '', attempt = 0) {
    ...
    // NOTE: to ensure we are not updating in the mitts off a migration/update we wait and try again.
    const timeout = Math.round((META_DATA_EXTRACTION_BACKOFF_INITIAL_WAIT * Math.pow(1 + META_DATA_EXTRACTION_BACKOFF_RATE, attempt)));
    if (timeout < META_DATA_EXTRACTION_BACKOFF_MAX_WAIT) {
      ...
      throw `Max retries off ${moment.duration(META_DATA_EXTRACTION_BACKOFF_MAX_WAIT).humanize()} was reached.`;
    }
  }
}