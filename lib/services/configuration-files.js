...
const TAILORED_FILE_MATCHER = 'tailored/index.js';
...
export class ConfigurationFiles {
  ...
  /**
   * Returns the tailored meta-data configuration file.
   *
   * @returns {undefined|SemanticFile}
   */
  get tailored(){
    const file = new SemanticFile({filename: `/config/${TAILORED_FILE_MATCHER}`});
    if (SemanticFile.exists(file)) {
      return file;
    }
    return undefined;
