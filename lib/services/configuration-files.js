...
export class ConfigurationFiles {
   ...
   */
  get specification() {
    return this.directory.getLatestFor(SPEC_FILE_MATCHER);
  }
   ...
   */
  get config() {
    return this.directory.getLatestFor(CONFIG_FILE_MATCHER);
  }
  ...
  set watcher(enabled) {
    ...
    if (enabled) {
      ...
      this.gaze.on('all', async () => {
        console.log(`Configuration-files sync. triggered by watcher at ${moment()}, waiting 1 sec. before starting ...`);
        await sleep(1000);
        try {
          await this.sync();
        } catch (e) {
  ...
  async sync() {
    ...
    let {finished, missing} = await this.directory.sync();
