...
export class SemanticFormManagement {
   ...
   */
  async getSemanticForm(uuid, options) {
    const forms = await getSemanticFormsInStore(uuid, options);
    if (forms.length) {
      if (forms.length > 1) {
        /**
  ...
  /**
   * Returns a collection of the latest sources to be used by the forms.
   *
   * @returns {(SemanticFile)[]}
   */
  getLatestSources() {
    return [
      this.configuration.specification.turtle,
      this.configuration.specification.json,
      this.meta.latest,
    ];
  }
  ...
  async getSemanticFormBundle(uuid) {
    ...
    /**
     * NOTE: in case no sources were set (new semantic-form)
     */
    if (!form.sources.length) {
      form.sources = this.getLatestSources();
      ...
      if (this.configuration.tailored.meta) {
        const extractors = this.configuration.tailored.meta.content;
...
async function getSemanticFormsInStore(uuid, options) {
  let response;
  try {
    const stringQuery = `
