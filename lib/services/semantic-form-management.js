...
export class SemanticFormManagement {
   ...
   *
   * TODO: this function has become very large, with lots of edge cases and is integral to the management of forms.
   *       it should be improved.
   *       for ex by adding extraction strategies based on the status of the form,
   *       to ease readability and improve exception handeling.
   *
   * [CONSIDERED AUTH. SAFE]
   *
   * @param uuid
    ...
    if (form.sources.length === 1) {
      ...
      if (configuration.tailored.meta) {
        ...
        if (additions) {
          ...
          try {
            await this.mutator.insert(file.toNT(), COMMON_GRAPHS.public);
          } catch (e) {
            // TODO: silent exception
    ...
    if (form.status === SEMANTIC_FORM_STATUS.NEW) {
      ...
      if (configuration && configuration.tailored.source) {
        ...
        try {
          ...
          /**
           * TODO: silent exception.
           *       was designed to be, under the impression that a next attempt would just
           *       try to relocated sources and save them.
           */
          console.warn(`Failed updating sources for semantic-form with ${uuid}`);
          console.log(e);
        }
