...
export class SemanticFormManagement {
  ...
  async getSemanticFormBundle(uuid) {
    ...
    if (!form.sources.length === 1) {
      // TODO: make this a little more type safe
      const {configuration, meta} = this.configuration.sources.getLatest(form.sources[0].uri);
      if(!configuration && !meta){
        throw {
          ...
          message: "Could not retrieve/find form configuration for the requested semantic-form"
        };
      }
      form.sources = [
