...
export class SemanticFormManagement {
  ...
  constructor(config_files, meta_data) {
    this.config_files = config_files;
    this.meta_data = meta_data;
    this.source = new SourceDataExtractor(this.config_files);
  ...
  async getSemanticFormBundle(uuid) {
    ...
    if (!form.sources.length) {
      ...
      form.sources.push(this.meta_data.latest);
      ...
      bundle.source = await this.source.extract(form.uri, form.getSource(CONFIG_FILE_MATCHER).content['resource']);
