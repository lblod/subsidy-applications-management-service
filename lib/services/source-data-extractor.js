...
/**
 * Service responsible for extracting source-data from the store.
 */
export class SourceDataExtractor {
  ...
  /**
   * Generate source-data for the given URI and definition.
   *
   ...
   */
  async extract(uri, definition = this.config_files.config.content['resource']) {
    try {
...
/**
 * Parse the pre-defined more user definition into what we need to construct the query.
 *
