...
WHERE {
    ...
    if (response) {
       ...
       */
      if (!binding.source) {
        formPOJO.source = this.versionService.active.uri;
        this.mutationService.update(formPOJO.graph, formPOJO.toNT()); // NOTE: no need to wait
      } else {
  ...
  async submitSemanticFormFor(uuid) {
    ...
    if (isValid(form_data)) {
      try {
        ...
        await this.mutationService.delete(form_data.resource.graph, form_data.resource.toNT());
        form_data.resource.status = SUBMISSION_STATUSES.sent;
        await this.mutationService.insert(form_data.resource.graph, form_data.resource.toNT());
      } catch (e) {
        console.warn(e);
        throw {
          status: 500,
