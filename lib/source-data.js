...
import { Graph, parse, RDFNode, resultToNT, serialize } from './util/rdflib';
...
WHERE {
  GRAPH ?g {
    ...
    if (result.results.bindings.length) {
      ...
      const ttl = bindings.map(b => resultToNT(b['s'], b['p'], b['o'])).join('\n');
 ...
 *  also might take a closer look at an issue we encountered in "search-query-management-service"
