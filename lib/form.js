...
import { getActiveFormData } from './form-data';
...
// TODO make more generic by calling it Form?
export class Form {
  ...
  async init(uuid) {
    ...
    if (!this.rdflibForm) {
      const active = await getActiveFormData();
      // TODO need to find a better way off updating and keeping the local store in sync
      const additions = `<${this.uri}> ${DCT('source')} <${active.uri}> .`;
      this.update({additions});
