...
export class SemanticForm {
  ...
  toNT() {
    ...
    buffer.push(`<${this.uri}> a <${this.type}> .`); // TODO redo with proper escape
    if (this.uuid)
      buffer.push(`<${this.uri}> mu:uuid ${sparqlEscape(this.uuid, 'string')} .`); // TODO redo with proper escape
    if (this.status)
      buffer.push(`<${this.uri}> adms:status <${this.status}> .`); // TODO redo with proper escape
    this.sources.forEach(source => {
      buffer.push(`<${this.uri}> dct:source <${source.uri}> .`); // TODO redo with proper escape
    });
