import { any, statements } from '../util/database';
  ...
  async build() {
    ...
    for (const [path, config] of Object.entries(this.routes)) {
      try {
        ...
        let objects = [];
        try {
          objects = await statements(`<${this.root}>`, `${path}`, undefined, {prefixes: this.prefixes, sudo: true});
        } catch (e) {
        ...
        if (objects.length) {
          ...
          objects.forEach(object => strategy.map(object, model));
        } else {
          console.log(`${path} was ignored. Reason: no objects found for path`);
        }
      } catch (e) {
