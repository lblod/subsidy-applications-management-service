    ...
    for (const directions of mapping) {
      try {
        ...
        try {
          ...
          objects = await statements(`<${root}>`, `${directions.from}`, undefined, options); // TODO redo with proper escape
        } catch (e) {
        ...
        if (objects.length) {
          ...
          console.log(
              `Mapping for <${root}> from ${directions.from} was ignored.\nReason: no objects found for given mapping directions`); // TODO redo with proper escape
        }
      } catch (e) {
        throw new Error(`Mapping for <${root}> from ${directions.path}.\nReason: ${e.message}`); // TODO redo with proper escape
      }
    }
    return this.model;
  }
